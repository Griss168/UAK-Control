...
const LOCAL_ASSETS = [
  '/UAK-Control/uak.html',
  '/UAK-Control/manifest.json',
  '/UAK-Control/icons/icon-192.png',
  '/UAK-Control/icons/icon-512.png',
];
...
self.addEventListener('fetch', e => {
  ...
  if (e.request.mode === 'navigate') {
    e.respondWith(
      caches.open(CACHE).then(cache => cache.match('/UAK-Control/uak.html'))
    );
    return;
  }
